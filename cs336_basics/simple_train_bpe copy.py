...
import regex
from cs336_basics.simple_bpe import (
    merge,
    BPETokenizerParams,
...
def train_bpe(
    string: str, num_merges: int, special_tokens: list[str] = None
    ...
    # Handle special tokens first (before pre-tokenization)
    special_tokens_set = set(special_tokens) if special_tokens else set()

    if special_tokens_set:
        # Split text on special tokens, preserving the special tokens as separate parts
        text_parts = _split_on_special_tokens(string, special_tokens_set)
        all_pre_tokens = []

        for part in text_parts:
            if part in special_tokens_set:
                # This is a special token, add it directly to pre_tokens
                all_pre_tokens.append(part)
            else:
                # This is regular text, apply GPT-2 pre-tokenization
                pre_tokens = regex.findall(GPT2_TOKENIZER_REGEX, part)
                all_pre_tokens.extend(pre_tokens)

        pre_tokens = all_pre_tokens
    else:
        # No special tokens, apply GPT-2 pre-tokenization directly
    ...
    return BPETokenizerParams(
        vocab=vocab,
        merges=merges,
        special_tokens=special_tokens_dict if special_tokens_dict else None,
    )
